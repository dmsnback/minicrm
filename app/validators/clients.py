...
async def validate_unique_email_client(session: AsyncSession, email: str) -> None:
...
async def validate_unique_phone_client(session: AsyncSession, phone: str) -> None:
...
async def validate_unique_full_name_client(session: AsyncSession, full_name: str) -> None:
    "Проверка уникальности имени клиента"
    result = await session.execute(select(Client).where(Client.full_name == full_name))
    if result.scalars().first():
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
